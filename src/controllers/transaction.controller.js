...
const { buyPosition, sellPosition } = require("../helpers/buyAndSell");
const coinbaseProLib = require("../helpers/coinbaseProLibrary");
...
const sellPositionDelta = Number(process.env.SELL_POSITION_DELTA) || 0.001; //The amount of change between peak and valley to trigger a sell off
const buyPositionDelta = Number(process.env.BUY_POSITION_DELTA) || 0.001 //The amount of change between the valley and peak price to trigger a buy in
...
async function momentumStrategyStart() {
  logger.debug(`Starting momentum strategy`);
  try {
...
exports.sell = async (req, res) => {
  ...
  const depositConfig = {
      ...
      depositingAmount: req.body.depositingAmount
  };

  const productInfo = {
    baseCurrency: req.body.baseCurrencyName,
    quoteCurrency: req.body.quoteCurrencyName,
    productPair: baseCurrencyName + "-" + quoteCurrencyName
  };
...
exports.buy = async (req, res) => {
  ...
  let lastValleyPrice;
  let highestFee = await returnHighestFee();
  let tradeBalance = Number.parseFloat(req.body.amount);
  ...
  if (availableBalance > balanceMinimum + tradeBalance) {
